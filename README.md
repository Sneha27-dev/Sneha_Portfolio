# 🌐 Sneha Ganiga — Personal Portfolio

[![Live Website](https://img.shields.io/badge/Live%20Website-Visit%20Now-c8925a?style=for-the-badge&logo=vercel)](https://sneha-portfolio-silk-seven.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/sneha-ganiga-557181312)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github)](https://github.com/Sneha27-dev)

---

## 👩‍💻 About Me

I'm a **Information Science and Engineering** graduate (CGPA: 9.02) from Vivekananda Institute of Technology, Bangalore.  
I'm passionate about **Data Analytics**, **Frontend Development**, and **Software Engineering** — building solutions that turn data into decisions and ideas into products.

---

## 🚀 Live Portfolio

🔗 **https://sneha-portfolio-silk-seven.vercel.app/**

---

## 🛠️ Built With

| Technology | Purpose |
|---|---|
| HTML5 | Structure |
| CSS3 | Styling & Animations |
| JavaScript | Interactivity & Scroll Effects |
| EmailJS | Contact Form |
| Font Awesome | Icons |
| Google Fonts | Typography (Playfair Display, DM Sans) |

---

## ✨ Features

- 🎨 Elegant warm cream & charcoal design
- 📱 Fully responsive (mobile & desktop)
- 🌀 Smooth scroll reveal animations
- 🖱️ Custom cursor effect
- 📩 Working contact form via EmailJS
- 📄 Resume download button
- 🌙 Clean professional layout

---

## 📁 Project Structure
- portfolio/
- ├── index.html           # Main HTML structure
- ├── style.css            # All styling & animations
- ├── script.js            # JS interactions & effects
- ├── profile.jpg          # Profile photo
- └── Sneha_Resume.pdf     # Downloadable resume

---

## 📊 Skills

- **Data & Analytics:** SQL, Excel, Power BI, EDA, Data Visualization
- **Frontend:** HTML, CSS, JavaScript
- **Mobile:** Kotlin, Jetpack Compose
- **Tools:** GitHub, VS Code

---

## 💼 Internships

- 🤖 **Android App Dev using Gen AI** — MindMatrix *(May 2026)*
- 📊 **Data Analyst Intern** — Innobyte Services *(Apr 2026)*
- 🌐 **Web Development Intern** — Edunet Foundation *(Apr 2025)*

---

## 🗂️ Projects

| # | Project | Tech |
|---|---|---|
| 01 | AWS Wild Rydes — Serverless Ride System | AWS Lambda, DynamoDB, S3 |
| 02 | Accident Predictive Analysis with Real-Time Alert | Python, CNN, YOLO |
| 03 | petCare Chatbot | HTML, CSS, JavaScript |
| 04 |Amazon Sales Analysis | Power BI, Excel, SQL |
| 04 | Pratham-Chikitse — Emergency First-Aid App | Kotlin, Jetpack Compose |

---

## 🏆 Certifications

- Android App Development using Generative AI — **MindMatrix**
- Data Analyst Internship — **Innobyte Services**
- Building Modern Web Apps with MERN Stack — **Edunet Foundation**
- Data Structures and Algorithms using Java — **NPTEL**

---

## 📬 Contact

- 📧 snehaganiga1527@gmail.com
- 📍 Bangalore, India
- 💼 [LinkedIn](https://www.linkedin.com/in/sneha-ganiga-557181312)
- 💻 [GitHub](https://github.com/Sneha27-dev)

---

⭐ *If you like this portfolio, feel free to give it a star!*
